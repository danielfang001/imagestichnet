...
with tf.name_scope('dataset'):
    ...
    # TensorFlow 2.x compatible iteration
    for train_input_tensor, train_size_tensor in train_data_dataset:
        # Set the shape of your tensors
        train_input_tensor.set_shape([batch_size, height, width, 3*2])
        train_size_tensor.set_shape([batch_size, 2, 1])
        ...
        # Use your train_inputs and train_size for training
        # ...

        # Print or log the train inputs
        print('train inputs = {}'.format(train_inputs))

        # Depending on your use case, you might want to break the loop
        # after one iteration or let it run for the entire dataset
